...
u8 TP_Scan(u8 tp) {
  if (PEN == 0) //有按键按下
  {
    ...
    {
      tp_dev.x[0] =
          tp_dev.xfac * tp_dev.x[0] + tp_dev.xoff; //将结果转换为屏幕坐标
  ...
  while (1) //如果连续10秒钟没有按下,则自动退出
  {
    ...
    if ((tp_dev.sta & 0xc0) == TP_CATH_PRES) //按键按下了一次(此时按键松开了.)
    {
      ...
      case 4: //全部四个点已经得到
        ...
        tp_dev.xoff =
            (lcddev.width - tp_dev.xfac * (pos_temp[1][0] + pos_temp[0][0])) /
            2; //得到xoff

        tp_dev.yfac = (float)(lcddev.height - 40) /
                      (pos_temp[2][1] - pos_temp[0][1]); //得到yfac
        tp_dev.yoff =
            (lcddev.height - tp_dev.yfac * (pos_temp[2][1] + pos_temp[0][1])) /
            2;                                            //得到yoff
...
u8 TP_Init(void) {
  ...
  if (lcddev.id == 0X5510) //电容触摸屏
  {
    ...
    RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB | RCC_AHB1Periph_GPIOC |
                           ...
                           ENABLE); //使能GPIOB,C,F时钟

    // GPIOB1,2初始化设置
    GPIO_InitStructure.GPIO_Pin =
        GPIO_Pin_1 | GPIO_Pin_2; // PB1/PB2 设置为上拉输入
