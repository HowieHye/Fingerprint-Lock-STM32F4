...
void LCD_Init(void) {
  ...
  GPIO_InitStructure.GPIO_Pin = (3 << 0) | (3 << 4) | (7 << 8) |
                                (3 << 14);     // PD0,1,4,5,8,9,10,14,15 AF OUT
  ...
  readWriteTiming.FSMC_AddressSetupTime =
      0XF; //地址建立时间（ADDSET）为16个HCLK 1/168M=6ns*16=96ns
  ...
  } else if (lcddev.id == 0x6804) // 6804初始化
  {
    ...
    delay_ms(20);
    LCD_WR_REG(
        0XD0); // VCI1  VCL  VGH  VGL DDVDH VREG1OUT power amplitude setting
    LCD_WR_DATA(0X07);
    LCD_WR_DATA(0X42);
    LCD_WR_DATA(0X1D);
    ...
    LCD_WR_DATA(0X1a);
    LCD_WR_DATA(0X09);
    LCD_WR_REG(0XD2); // Operational Amplifier Circuit Constant Current Adjust ,
                      // charge pump frequency setting
    LCD_WR_DATA(0X01);
    LCD_WR_DATA(0X22);
  ...
  } else if (lcddev.id == 0x9328) // ILI9328   OK
  {
    ...
    LCD_WriteReg(0x0002, 0x0700); //电源开启
                                  ...
                                  // //65K  RGB DRIVE TABLE(寄存器 03H) BIT3=AM
                                  // BIT4:5=ID0:1 AM ID0 ID1   FUNCATION
                                  // 0  0   0	   R->L D->U
                                  // 1  0   0	   D->U	R->L
                                  // 0  1   0	   L->R D->U
                                  // 1  1   0    D->U	L->R
  ...
  } else if (lcddev.id == 0x9320) //测试OK.
  {
    ...
    LCD_WriteReg(0x12,
                 (1 << 8) | (1 << 4) | (0 << 0)); // Power Control 3.(0x0138)
  ...
  {
    // second release on 3/5  ,luminance is acceptable,water wave appear during
    // camera preview
    LCD_WriteReg(0x0007, 0x0000);
    delay_ms(50);
    ...
    LCD_WriteReg(0x0012, 0x011E); // 0x011c  //Vreg1out = Vcilvl*1.80   is it
                                  // the same as Vgama1out ?
    LCD_WriteReg(0x0013,
