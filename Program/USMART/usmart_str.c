  ...
  while (1) {
    ...
    } else //已经接受完了函数名了.
    {
      if (*strtemp == ',') {
        ...
        if (pcnt == 0 &&
            fplcnt <
                5) //当第一个参数来时,为了避免统计void类型的参数,必须做判断.
        {
  ...
  while (1) {
    ...
    case 0:                //数字
      if (tstr[0] != '\0') //接收到的参数有效
      {
        ...
        *(u32 *)(usmart_dev.parm + usmart_get_parmpos(n)) =
            res;                          //记录转换成功的结果.
    ...
    case 1:                          //字符串
      ...
      usmart_strcopy(tstr, &usmart_dev.parm[usmart_get_parmpos(
                               n)]); //拷贝tstr数据到usmart_dev.parm[n]
